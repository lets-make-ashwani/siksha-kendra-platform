  ...
  try {
    ...
    try {
      await transporter.sendMail({
        from: process.env.EMAIL_FROM || 'noreply@sikshakendra.com',
        to: user.email,
        subject: 'Siksha Kendra - Vendor Account Approved!',
        html: `<div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 8px;"> <h2 style="color: #FF6B00;">Congratulations, ${user.name}!</h2> <p>Your application to become a Siksha Kendra Vendor has been <strong>approved</strong>.</p> <p>Here are your official details:</p> <ul> <li><strong>Vendor ID:</strong> ${vendor_id}</li> <li><strong>Referral Code:</strong> ${referral_code}</li> </ul> <p>You can now log into your dashboard and start referring students:</p> <p style="background: #f4f4f4; padding: 10px; border-radius: 5px;"> <strong>Login Email:</strong> ${user.email}<br/> <strong>Temporary Password:</strong> ${plainPassword} </p> <a href="${process.env.FRONTEND_URL}/login" style="display: inline-block; padding: 10px 20px; background: #FF6B00; color: white; text-decoration: none; border-radius: 5px; margin-top: 15px;">Login to Dashboard</a> </div>`
      });
    } catch (emailError) {
